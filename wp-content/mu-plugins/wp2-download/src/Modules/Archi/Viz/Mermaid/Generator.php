 ...
 *
 * @package WP2\Download\Modules\Archi\Viz\Mermaid
 */
...
final class Generator
{
    ...
    private static function build_facet_line(array $facet, bool $use_colon_syntax): string
    {
        ...
        $is_method = isset($facet['parameters']) && is_array($facet['parameters']);

        if ($is_method) {
            return self::build_method_line(
                $visibility,
                $name,
                $generic,
                $classifier,
        ...
        return self::build_property_line(
            $visibility,
            $name,
            $base_type,
            $generic,
    ...
    /**
     * Builds a method line for Mermaid syntax.
     *
     ...
     * @param bool   $use_colon_syntax
     * @param string $return_type
     * @return string
     */
    private static function build_method_line(
        string $visibility,
        string $name,
        string $generic,
        string $classifier,
        array $parameters,
        bool $use_colon_syntax,
        string $return_type
    ): string {
        $params = array_map(
            fn ($p) => self::escape_text($p['name'] ?? ''),
            $parameters
        );
    ...
    /**
     * Builds a property line for Mermaid syntax.
     *
     * @param string $visibility
     * @param string $name
     ...
     * @return string
     */
    private static function build_property_line(
        string $visibility,
        string $name,
        string $base_type,
        string $generic,

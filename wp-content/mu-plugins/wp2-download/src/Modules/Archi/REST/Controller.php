<?php
namespace WP2\Download\Archi\REST;
...
final class Controller extends WP_REST_Controller {
		...
		register_rest_route(
			...
			[ 
				[ 
					...
					'args' => [ 
						'facet' => [ 'type' => 'string', 'required' => false, 'description' => 'Filter components by a specific facet.' ],
						'format' => [ 'type' => 'string', 'required' => false, 'enum' => [ 'json', 'mermaid' ], 'default' => 'json' ],
						'compact' => [ 'type' => 'boolean', 'required' => false, 'description' => 'For Mermaid format, generate a compact diagram.' ],
					],
				],
			]
		);
	...
	public function get_graph( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( 'mermaid' === $format ) {
			...
			return new WP_REST_Response( [ 'format' => 'mermaid', 'diagram' => $mermaid ], 200 );
		}
		...
		return new WP_REST_Response( [ 'format' => 'json', 'nodes' => $nodes, 'edges' => $edges ], 200 );
	}
}

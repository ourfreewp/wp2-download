...
class Scheduler
{
    public const MAIN_HOOK = 'wp2_schedule_all_health_checks';
    public const INDIVIDUAL_HOOK = 'wp2_run_individual_health_check';
    public const ACTION_GROUP = 'wp2-download-health';

    public function register_hooks(): void
    {
        // Hook into Action Scheduler's initialization.
        add_action('action_scheduler_init', [$this, 'schedule_main_event']);

        // Add the handler for the main event.
        add_action(self::MAIN_HOOK, [$this, 'spawn_individual_check_actions']);

        // Add the handler for the individual check events.
        add_action(self::INDIVIDUAL_HOOK, [$this, 'handle_individual_check_action'], 10, 2);
    }
    ...
    public function schedule_main_event(): void
    {
        ...
        if (!as_has_scheduled_action(self::MAIN_HOOK, [], self::ACTION_GROUP)) {
            as_schedule_recurring_action(time(), DAY_IN_SECONDS, self::MAIN_HOOK, [], self::ACTION_GROUP);
        }
    }
    ...
    public function spawn_individual_check_actions(): void
    {
        $packages_query = new \WP_Query(
            [
                'post_type' => ['wp2_plugin', 'wp2_theme', 'wp2_mu'],
                'posts_per_page' => -1,
                'fields' => 'ids',
            ]
        );

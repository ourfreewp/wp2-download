...
class Runner
{
    ...
    public function run_check(string $check_id, int $post_id): void
    {
        ...
        if (!$post || !isset($this->checks[$check_id])) {
            \WP2\Download\Utils\Logger::log('Runner: Invalid post or check_id for run_check: ' . $check_id . ', post_id=' . $post_id, 'ERROR');
            return;
        }

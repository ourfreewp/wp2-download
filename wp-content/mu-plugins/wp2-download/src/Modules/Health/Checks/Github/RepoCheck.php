 ...
 *
 * @component_id health_github_repo_check
 * @namespace health.checks.github
 * @type Check
 * @note "Checks GitHub repository status."
 */
class RepoCheck extends BaseCheck
{
    ...
    protected function perform_check(bool $force = false)
    {
        ...
        \WP2\Download\Utils\Logger::log("GitHubCheck: repo_url=$repo_url", 'DEBUG');

        if (!$repo_url || !preg_match('/^https?:\/\/github\.com\/([^\/]+\/[^\/]+)$/', $repo_url, $matches)) {
            \WP2\Download\Utils\Logger::log('GitHubCheck: Invalid or missing repo_url for post_id=' . ($this->package_post->ID ?? 'unknown'), 'ERROR');
            return [
                'status' => 'error',
        ...
        if ($cached_data !== false && $force !== true) {
            \WP2\Download\Utils\Logger::log("GitHubCheck: Using cached data for $repo_slug", 'DEBUG');
        ...
        \WP2\Download\Utils\Logger::log("GitHubCheck: Preparing API request to $api_url for repo_url=$repo_url", 'DEBUG');
        $args = ['timeout' => 15];
        if (defined('WP2_GITHUB_PAT') && WP2_GITHUB_PAT) {
            $args['headers'] = ['Authorization' => 'Bearer ' . WP2_GITHUB_PAT];
            \WP2\Download\Utils\Logger::log("GitHubCheck: Using PAT for $repo_slug", 'DEBUG');
        } else {
            \WP2\Download\Utils\Logger::log("GitHubCheck: No PAT defined for $repo_slug", 'WARNING');
        }

        \WP2\Download\Utils\Logger::log("GitHubCheck: API request url=$api_url args=" . wp_json_encode($args), 'DEBUG');
        $response = wp_remote_get($api_url, $args);

        \WP2\Download\Utils\Logger::log("GitHubCheck: API response for $api_url: " . wp_json_encode($response), 'DEBUG');

        if (is_wp_error($response)) {
            $error_msg = $response->get_error_message();
            \WP2\Download\Utils\Logger::log("GitHub API ERROR for {$repo_slug}: $error_msg | Args: " . wp_json_encode($args), 'ERROR');
            return [
                'status' => 'error',
        ...
        if ($http_code !== 200) {
            \WP2\Download\Utils\Logger::log("GitHub API HTTP ERROR for {$repo_slug}: HTTP $http_code | Args: " . wp_json_encode($args), 'ERROR');
            \WP2\Download\Utils\Logger::log("GitHub API response body for {$repo_slug}: " . wp_remote_retrieve_body($response), 'ERROR');
            return [
                'status' => 'error',
        ...
        \WP2\Download\Utils\Logger::log('GitHubCheck: API success data=' . wp_json_encode($data), 'DEBUG');
    ...
    private function update_meta_from_data(array $data, string $repo_url = ''): void
    {
        ...
        error_log('GitHubCheck: API success data=' . wp_json_encode($data));

...
class Controller {
	public function register_routes() {
		add_action( 'rest_api_init', function () {
			register_rest_route( 'wp2/v1', '/ingest-release', [ 
				'methods' => 'POST',
	...
	public function handle_ingest_release( $request ) {
		...
		$parent_id = $parent_post ? $parent_post->ID : wp_insert_post( [ 'post_type' => $parent_post_type, 'post_title' => $slug, 'post_name' => $slug, 'post_status' => Config::WP2_POST_STATUS_PUBLISH ] );
		if ( is_wp_error( $parent_id ) || ! $parent_id ) {
			return new \WP_REST_Response(
				[ 'error' => [ 'code' => 'parent_package_error', 'message' => 'Could not find or create parent package.' ] ],
				500
			);
		}
		$release_id = wp_insert_post( [ 'post_type' => Config::WP2_POST_TYPE_PLUGIN_REL, 'post_parent' => $parent_id, 'post_title' => "{$slug} - {$version}", 'post_status' => Config::WP2_POST_STATUS_PUBLISH ] );
		if ( is_wp_error( $release_id ) ) {
			return new \WP_REST_Response(
				[ 'error' => [ 'code' => 'release_post_error', 'message' => 'Could not create release post.' ] ],
				500
			);
		}
		...
		return new \WP_REST_Response( [ 'success' => true, 'release_id' => $release_id ], 201 );
	}
}

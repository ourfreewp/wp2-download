...
class Controller {
	public function register_routes() {
		add_action( 'rest_api_init', function () {
			register_rest_route( 'wp2/v1', '/packages/(?P<type>[a-z\-]+)/(?P<slug>[a-z0-9\-]+)', [ 
				'methods' => 'GET',
				'callback' => [ $this, 'get_latest_package_info' ],
	...
	public function get_latest_package_info( $request ) {
		...
		if ( ! $release_query->have_posts() ) {
			return new \WP_REST_Response(
				[ 'error' => [ 'code' => 'no_releases_found', 'message' => 'No releases found.' ] ],
				404
			);
		}

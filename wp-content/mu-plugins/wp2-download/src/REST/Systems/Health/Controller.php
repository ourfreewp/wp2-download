...
namespace WP2\Download\REST\Systems\Health;

use WP2\Download\Admin\Manifest;
use WP2\Download\Services\Locator;

/**
 * @component_id rest_system_health_controller
 * @namespace rest.system.health
 * @type Controller
 * @note "Handles REST API routes for health checks."
 */
class Controller {
	...
	public function register_routes() {
		add_action(
			...
			function () {
				...
				register_rest_route(
					'wp2/v1',
					'/purge_processed_manifests',
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'purge_processed_manifests' ),
						'permission_callback' => array( $this, 'permission_callback' ),
					)
				);
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'sync_to_r2' ),
						'permission_callback' => array( $this, 'permission_callback' ),
					)
				);
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'create_release' ),
						'permission_callback' => array( $this, 'permission_callback' ),
					)
				);
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'edit_manifest' ),
						'permission_callback' => array( $this, 'permission_callback' ),
					)
				);
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'purge_artifact' ),
						'permission_callback' => array( $this, 'permission_callback' ),
					)
				);
			}
	...
	public function run_all_health_checks( \WP_REST_Request $request ) {
		...
		return new \WP_REST_Response(
			array(
				'success' => true,
				'message' => 'Triggered health checks.',
			),
			200
		);
	}
	...
	public function purge_processed_manifests( \WP_REST_Request $request ) {
		...
		return new \WP_REST_Response(
			array(
				'success' => true,
				'message' => 'Processed manifests option purged.',
			),
			200
		);
	}

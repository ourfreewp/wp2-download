...
class Adapter implements ConnectionInterface
{
    ...
    {
        // Helper to resolve config value by precedence: constant → env → config → default.
        $resolve = function ($key, $default = '') use ($config) {
            $const = 'WP2_R2_' . strtoupper($key);
            if (defined($const)) {
        ...
        $this->account_id = $resolve('account_id');
        $this->bucket = $resolve('bucket');
        $this->access_key_id = $resolve('access_key_id');
        $this->secret_access_key = $resolve('secret_access_key');

...
/**
 * Class GitHubTagCheck
 * Health check for detecting new, unreleased tags in GitHub repositories.
 *
...
class TagCheck extends BaseCheck {
	...
	protected function perform_check( bool $force = false ) {
		...
		if ( empty( $tags ) || ! isset( $tags[0]['name'] ) ) {
			$result = [ 'status' => 'success', 'data' => [ 'latest_tag' => null, 'is_pending' => false ] ];
		...
		$result = [ 'status' => 'success', 'data' => [ 'latest_tag' => $latest_tag, 'is_pending' => $is_pending ] ];
		set_transient( $transient_key, $result, HOUR_IN_SECONDS );
		return $result;
	}

...
/**
 * @component_id admin_releases
 * @namespace admin
 * @type Page
 * @note "Releases admin page with native tab navigation."
 */
class Releases {
	...
	/**
	 * Enqueue assets for the Releases page.
	 *
	 * @param string $hook Current admin page hook.
	 * @return void
	 */
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-releases' !== $hook && 'wp2-hub-releases' !== $hook ) {
			return;
		}
	...
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/releases/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/releases.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_releases_tab_order', array_keys( $tabs ) );
		$ordered_tabs = [];
		 ...
		 */
		return apply_filters( 'wp2_download_admin_releases_tabs', $ordered_tabs );
	}

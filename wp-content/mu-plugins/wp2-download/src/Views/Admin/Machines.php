...
/**
 * @component_id admin_machines
 * @namespace admin
 * @type Page
 * @note "Machines admin page with native tab navigation."
 */
class Machines {
	...
	public function register_hooks(): void {
		...
		add_filter( 'wp2_download_admin_machines_tab_order', function ($order) {
	...
	public function add_submenu_page(): void {
		add_submenu_page(
			'wp2-hub',
			__( 'WP2 Hub Machines', 'wp2-download' ),
			__( 'Machines', 'wp2-download' ),
			'manage_options',
			'wp2-hub-machines',
	...
	/**
	 * Enqueue assets for the machines page.
	 *
	 * @param string $hook Current admin page hook.
	 * @return void
	 */
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-machines' !== $hook && 'wp2-hub-machines' !== $hook ) {
			return;
		}
	...
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/machines/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/machines.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_machines_tab_order', array_keys( $tabs ) );
		$ordered_tabs = [];
		...
		/**
		 * Filter the discovered machines tabs.
		 *
		 * @param array<string,array{label:string,file:string}> $tabs Tabs map.
		 */
		return apply_filters( 'wp2_download_admin_machines_tabs', $ordered_tabs );
	}

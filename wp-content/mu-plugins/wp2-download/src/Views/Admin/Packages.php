...
/**
 * @component_id admin_packages
 * @namespace admin
 * @type Page
 * @note "Packages admin page with native tab navigation."
 */
class Packages {
	...
	public function register_hooks(): void {
		...
		add_filter( 'wp2_download_admin_packages_tab_order', function ($order) {
			return [ 'overview', 'plugins', 'themes', 'mu-plugins' ];
		} );
	}
	...
	/**
	 * Enqueue assets for the Packages page.
	 *
	 * @param string $hook Current admin page hook.
	 * @return void
	 */
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-packages' !== $hook && 'wp2-hub-packages' !== $hook ) {
			return;
		}
	...
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/packages/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/packages.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_packages_tab_order', array_keys( $tabs ) );
		$ordered_tabs = [];
		...
		/**
		 * Filter the discovered audit tabs.
		 *
		 * @param array<string,array{label:string,file:string}> $tabs Tabs map.
		 */
		return apply_filters( 'wp2_download_admin_packages_tabs', $ordered_tabs );
	}

...
/**
 * Audits admin page with native tab navigation.
 */
class Audits {
	...
	public function register_hooks(): void {
		...
		add_filter( 'wp2_download_admin_audits_tab_order', function ($order) {
	...
	public function add_submenu_page(): void {
		add_submenu_page(
			'wp2-hub',
			__( 'WP2 Hub Audits', 'wp2-download' ),
			__( 'Audits', 'wp2-download' ),
			'manage_options',
			'wp2-hub-audits',
	...
	/**
	 * Enqueue assets for the audits page.
	 *
	 * @param string $hook Current admin page hook.
	 * @return void
	 */
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-audits' !== $hook && 'wp2-hub-audits' !== $hook ) {
			return;
		}
	...
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/audits/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/audits.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_audits_tab_order', array_keys( $tabs ) );
		$ordered_tabs = [];
		 ...
		 */
		return apply_filters( 'wp2_download_admin_audits_tabs', $ordered_tabs );
	}

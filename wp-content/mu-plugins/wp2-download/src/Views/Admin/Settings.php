...
/**
 * @component_id admin_settings
 * @namespace admin
 * @type Page
 * @note "Settings admin page with native tab navigation."
 */
class Settings {
	...
	public function register_hooks(): void {
		...
		add_filter( 'wp2_download_admin_settings_tab_order', function ($order) {
	...
	public function add_submenu_page(): void {
		add_submenu_page(
			'wp2-hub',
			__( 'WP2 Settings', 'wp2-download' ),
			__( 'Settings', 'wp2-download' ),
			'manage_options',
			'wp2-hub-settings',
	...
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-settings' !== $hook && 'wp2-hub-settings' !== $hook ) {
			return;
		}
	...
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/settings/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/settings.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_settings_tab_order', array_keys( $tabs ) );
		 ...
		 */
		return apply_filters( 'wp2_download_admin_settings_tabs', $ordered_tabs );
	}

...
/**
 * @component_id admin_origins
 * @namespace admin
 * @type Page
 * @note "Origins admin page with native tab navigation."
 */
class Origins {
	...
	public function register_hooks(): void {
		...
		add_filter( 'wp2_download_admin_origins_tab_order', function ($order) {
			return [ 'settings', 'storage', 'composer', 'wporg', 'github', 'gdrive' ];
		} );
	}
	...
	public function add_submenu_page(): void {
		add_submenu_page(
			'wp2-hub',
			__( 'WP2 Hub Origins', 'wp2-download' ),
			__( 'Origins', 'wp2-download' ),
			'manage_options',
			'wp2-hub-origins',
	...
	/**
	 * Enqueue assets for the origins page.
	 *
	 * @param string $hook Current admin page hook.
	 * @return void
	 */
	public function enqueue_assets( string $hook ): void {
		if ( 'wp2-hub_page_wp2-hub-origins' !== $hook && 'wp2-hub-origins' !== $hook ) {
			return;
		}
	...
	/**
	 * Render the origins page.
	 *
	 * @return void
	 */
	public function render_page_contents(): void {
		$views_dir = WP2_DOWNLOAD_PATH . 'src/Admin/views/origins/';
		...
		require WP2_DOWNLOAD_PATH . 'src/Admin/views/origins.php';
	}
	...
	private function get_tabs( string $views_dir ): array {
		...
		$order = apply_filters( 'wp2_download_admin_origins_tab_order', array_keys( $tabs ) );
		$ordered_tabs = [];
		...
		/**
		 * Filter the discovered origin tabs.
		 *
		 * @param array<string,array{label:string,file:string}> $tabs Tabs map.
		 */
		return apply_filters( 'wp2_download_admin_origins_tabs', $ordered_tabs );
	}

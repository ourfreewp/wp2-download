<?php
/**
 * Accounts page wrapper and tab navigation.
 *
...
<div class="wrap wp2-accounts">
	<h1><?php echo esc_html__( 'WP2 Accounts', 'wp2-download' ); ?></h1>
	<p class="description">
		<?php echo esc_html__( 'Review the status and detailed data for each service connection.', 'wp2-download' ); ?>
	</p>
	...
	<?php if ( ! empty( $tabs ) ) : ?>
		...
		<div class="wp2-accounts__tab-panel" role="tabpanel"
			aria-labelledby="<?php echo esc_attr( 'tab-' . $current_tab ); ?>">
			<?php $render_tab( $current_tab ); ?>
		</div>
	<?php else : ?>
		<div class="notice notice-warning">
			<p><?php echo esc_html__( 'No account tabs found.', 'wp2-download' ); ?></p>
		</div>
	<?php endif; ?>
</div>